...
if __name__ == '__main__':
    ...
    tree = dendropy.Tree.get(path=files_for_input[9],
                             schema="newick", preserve_underscores=True)

    ###############################################################################
    ## Load up the csv for the checking ###########################################
    ###############################################################################
    ...
    for taxon in tree.taxon_namespace:
        ...
        # second_occy = find_nth(current_name_string, "_",2)
        # new_name = current_name_string[0:second_occy]
        ...
        if indy == "Not_here":
            ...
            cluster_val = cluster_csv.iloc[indy, cluster_csv.columns.get_loc("insert_name") ]
            cluster_num_col.append(int(cluster_val))
    ...
    tree = dendropy.Tree.get_from_path(files_for_input[9],
    ...
    ###############################################################################
    ## So now I have all the states for the tree I need to navigate it from a #####
    ## starting node to a finishing node with a different state, then compare #####
    ## the different number of children from each of these nodes ##################
    ###############################################################################

    ###############################################################################
    ## So this loop below looks from the tips of the isolates with the MGE at a ###
    ## certain insertion loci, backwards, working out which is the node that ######
    ## precedes the insertion of the element via its inferred state not matching ##
    ## the taxa's state ###########################################################
    ###############################################################################


    ## Lets label the individual nodes in the tree as well for our bookeeping
    ...
    #print("This is the number of nodes in the tree:::", counter)
    ## Initialize the out csv
    ...
    out_df['insertion_node'] = pandas.Series(node_home, index=out_df.index)
    out_df['node_rec'] = pandas.Series(reccy_on_node, index=out_df.index)

    out_df.to_csv(files_for_input[5])
    ...
    ###############################################################################
    ## Now we'll get the start and end nodes for the recombination events which ###
    ## we'll then use to highlight the specific reoombination events that bring ###
    ## in the MGEs ################################################################
    ###############################################################################

    ref_gff_tsv = pandas.read_csv(files_for_input[1], sep='\t',
                                  names=["type","prog","class","start","end","trent","alexander","arnold","attribute"],
                                  header=None)
    ...
    ###############################################################################
    ## Now we'll load up the tree so we can look at the previous node to the one ##
    ## in question for further work ###############################################
    ###############################################################################

    tree = dendropy.Tree.get(path = files_for_input[2],
                             schema="newick", preserve_underscores = True)
    ...
    python_hits_csv = pandas.read_csv(files_for_input[3])
    ...
    for k in range(len(node_count)):
        ...
        if len(check_if_same_insertion_loci) == 1:
            ...
            before_locs = noders.iloc[0,[3,4]]
            after_locs = noders.iloc[0,[6,7]]
            example_id = noders.iloc[0,11]
            ...
            for k in range(len(check_if_same_insertion_loci)):
                ...
                before_locs = subset_noders.iloc[0,[3,4]]
                after_locs = subset_noders.iloc[0,[6,7]]
                example_id = subset_noders.iloc[0,11]
    ...
    reccy_df.to_csv(files_for_input[4],
                    index=False)
    ...
    non_reccy_df.to_csv(files_for_input[5],
                       index=False)
